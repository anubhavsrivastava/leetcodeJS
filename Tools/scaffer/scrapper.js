  ...
  extract: async (leetCodeProblemURL) => {
    ...
    await page.waitFor("body").then(async () => {
      ...
      if (isSPA) {
          ...
          let problemData = await page.evaluate(
            async ({ UrlObj }) => {
              ...
              if (order1Difficulty.length) {
                ...
                result.difficulty = document
                  .querySelectorAll("span[data-diff]")[0]
                  .getAttribute("data-diff");
              }

              let order1Description = document.querySelectorAll(
                "div[class^='description'] >div"
              );
              if (order1Description.length) {
                result.completeProblemName =
                  order1Description[0].firstElementChild.innerText;
                result.question = order1Description[1].innerText;
              } else {
                result.completeProblemName = document.querySelectorAll(
                  `a[href="${UrlObj.pathname}"]`
                )[0].innerText;
                result.question = document.querySelectorAll(
        ...
        result = await page
          .waitFor(
            "div.inline-flex.items-center.justify-center.text-caption.gap-1.rounded-full.bg-fill-secondary"
          )
          .then(async () => {
            ...
            let problemData = await page.evaluate(
              async ({ UrlObj }) => {
                let result = {};
                result.difficulty = document.querySelectorAll(
                  "div.inline-flex.items-center.justify-center.text-caption.gap-1.rounded-full.bg-fill-secondary"
                )[0].innerText;
                result.completeProblemName = document.querySelectorAll(
                  "div.flex.items-start > div > a"
                )[0].innerText;
                // problemName: "872. Leaf-Similar Trees"
                let problemArr = result.completeProblemName.split(". ");
